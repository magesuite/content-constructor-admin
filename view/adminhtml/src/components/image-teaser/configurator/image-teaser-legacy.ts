                        ...
                        <div class="cc-image-teaser-legacy-configurator__item-col-right">
                            <div class="cc-input cc-input--group">
                                <div class="cc-input | cc-image-teaser-legacy-configurator__item-form-element">
                                    <label for="cfg-it-item{{ $index }}-variant" class="cc-input__label">${$t(
                                        'Display variant'
                                    )}:</label>
                                ...
                                <div class="cc-input | cc-image-teaser-legacy-configurator__item-form-element">
                                    <label for="cfg-it-item{{ $index }}-color-scheme" class="cc-input__label">${$t(
                                        'Text color scheme'
                                    )}:</label>
                            ...
                            <div class="cc-input cc-input--group">
                                ...
                                <div class="cc-input cc-input--type-addon | cc-image-teaser-legacy-configurator__item-form-element">
                                    <label for="image-teaser-ctatarget-output-{{ $index }}" class="cc-input__label">${$t(
                                        'CTA target link'
                                    )}:</label>
    ...
    methods: {
        _collectPossibleOptions(filteredScenarios: any[][]): any {
            ...
            filteredScenarios.forEach((filteredScenario: any[]) => {
                ...
                possibleOptions.textPositioning[
                    filteredScenario[textPositionIndex]
                ] = true;
                filteredScenario[mobileLayoutsIndex].forEach(
                    (mobileLayout: string) => {
        ...
        _findPossibleOptions(
            ...
            if (mobileLayout) {
                filteredScenarios = filteredScenarios.filter(
                    (availableScenario: any) => {
                        return (
                            availableScenario[mobileLayoutsIndex].indexOf(
                                mobileLayout
                            ) !== -1
                        );
                    }
        ...
        toggleOption(optionCategory: string, optionId: string): void {
            if (this.configuration.currentScenario[optionCategory].id) {
                ...
                this.configuration.currentScenario[
                    optionCategory
        ...
        adjustVisibleItems(): void {
            ...
            const itemsNumber: number = this.configuration.currentScenario
                .desktopLayout.teasersNum;
            const itemsLimit: boolean = this.configuration.currentScenario
            ...
            if (itemsLimit && items.length > itemsNumber) {
                ...
                this.configuration.ignoredItems = removedItems.concat(
                    this.configuration.ignoredItems
                );
            } else if (items.length < itemsNumber) {
                items.concat(
                    this.configuration.ignoredItems.splice(
                        0,
                ...
                for (
                    ...
                    addedItems < itemsNumber - items.length;
                    addedItems++
                ) {
        ...
        getImageUploader(index: number): void {
            MediabrowserUtility.openDialog(
                ...
                }target_element_id/image-teaser-img-${index}/`,
                'auto',
                'auto',
                $t('Insert File...'),
                {
        ...
        imageUploadListener(): void {
            ...
            $(document).on(
                'change',
                '.cc-image-teaser-legacy-configurator__image-url',
                (event: Event): void => {
                    if (!isAlreadyCalled) {
        ...
        moveImageTeaserUp(index: number): void {
            if (index > 0) {
                const $thisItem: any = $(
                    `#cc-image-teaser-legacy-item-${index}`
                );
                const $prevItem: any = $(
                    `#cc-image-teaser-legacy-item-${index - 1}`
                );

                $thisItem
                    .addClass(
                        'cc-image-teaser-legacy-configurator__item--animating'
                    )
                    .css(
                        'transform',
                        `translateY(${-Math.abs(
        ...
        widgetSetListener(): void {
            $('.cc-image-teaser-legacy-configurator__cta-target-link').on(
                'change',
                (): void => {
                    this.onChange();
                }
        ...
        isPossibleToDelete(index: number): Boolean {
            if (
                (index !== 0 || this.configuration.items.length > 1) &&
